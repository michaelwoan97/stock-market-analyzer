...
from data_processing.stock_data_processing import StockAnalyzerSQL
...
# Notes: because the SqlDatabaseOperator is invovled in database operations so allowed only one connection or one instance
class SqlDatabaseOperator:
    ...
    def create_companies_info_from_csv(self):
        sheet_name = "Stock"
        # XLSX file path
        xlsx_file_path = 'database-ops/Assets/Yahoo Ticker Symbols - September 2017.xlsx' 
        workbook = None
        cursor = None
        conn = None
    ...
    def create_stock_market(self):
        ...
        stocks_table = "Stocks"
        stock = "TSLA"
        print(f'Testing insert {stock} data into {stocks_table} table.....')
        data = stock_market_operator.process_stock_data(stock,"USA","2022-10-10","2023-10-10",False)
        if len(data):
            print(f'>>> {stock} data is inserted. ')
        ...
        print("======================================================================================================================")
        print(f'Testing get {stock}\'s technical data from view after calculations....')
        techincal_data = stock_market_operator.process_stock_data(stock,"USA","2022-10-10","2023-10-10",True)
        if len(techincal_data):
            print(f'>>> {stock}\'s technical data is ready. ')
        print("======================================================================================================================")

        print("======================================================================================================================")
        stock = "AAPL"
        print(f'Testing insert {stock}\'s technical data to calculation tables....')
        techincal_data = stock_market_operator.process_stock_data(stock,"USA","2022-10-10","2023-10-10",True)
        if len(techincal_data):
            print(f'>>> {stock}\'s technical data is inserted. ')

        print(f'Testing get {stock}\'s technical data by joinning tables....')
        techincal_data = stock_market_operator.process_stock_data(stock,"USA","2022-10-10","2023-10-10",True)
        if len(techincal_data):
            print(f'>>> {stock}\'s technical data is ready. ')
